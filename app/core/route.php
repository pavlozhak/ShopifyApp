...
class Router
{
    ...
    public function setGETparams($params)
    {
        ...
        if(sizeof($parts) > 0) {
            ...
            {
                $explode2keyAndValues = explode('=', $part);
                $_GET[$explode2keyAndValues[0]] = $explode2keyAndValues[1];
            }
        }
    }
    ...
    public function start()
	{
		...
		list($actionNameFromUriWithoutParams, $params) = explode('?', $actionNameFromURI);
		...
		// get action name
        $this->actionName = (empty($actionNameFromURI)) ? $this->defaultActionName : $actionNameFromUriWithoutParams;
